...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (['getNotifications', 'sendNotification', 'markNotificationRead'].includes(action)) {
        await pool.query(`
        ...
        if (isBroadcast) {
            ...
            const queries = users.map(u => {
                ...
                return pool.query(
                    ...
                    [uniqueId, u.id, title, message, type]
                );
            });
    ...
    if (action === 'register') {
      ...
      let finalRole = 'user';
      if (role === 'admin' && process.env.ADMIN_SECRET && adminSecret === process.env.ADMIN_SECRET) finalRole = 'admin';
      try {
        await pool.query(
    ...
    if (action === 'changePassword') {
        ...
        if (rows[0].password !== currentPassword) return res.status(401).json({ error: 'Incorrect current password' });
    ...
    if (action === 'recordTransaction') {
      ...
      if (type === 'product_purchase') {
          await pool.query('INSERT INTO purchases (user_id, product_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [userId, itemId]);
      } else if (type === 'course_enrollment') {
           await pool.query('INSERT INTO enrollments (user_id, course_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [userId, itemId]);
      }
      return res.status(200).json({ success: true });
    }
    ...
    if (action === 'getAdminStats') {
        ...
        return res.status(200).json({ totalRevenue: parseFloat(totalRevenue) });
    }
    ...
    if (action === 'getAllUsers') {
      ...
      const mapped = users.map((u: any) => ({
        ...
        balance: parseFloat(u.balance), status: u.status, photoUrl: u.photo_url,
        joinedAt: u.created_at, enrolledCourses: enrollMap.get(u.id) || []
      }));
      return res.status(200).json(mapped);
    }
    ...
    if (action === 'getLogs') {
        ...
        const mapped = rows.map((l: any) => ({
            id: l.id, userId: l.user_id, action: l.action, description: l.description, timestamp: l.created_at, type: l.type
        }));
        return res.status(200).json(mapped);
    }
    ...
    if (action === 'getCourses') {
      ...
      const mapped = rows.map((c: any) => ({ ...c, price: parseFloat(c.price), modules: c.modules_json || [] }));
      return res.status(200).json(mapped);
    }
    ...
    if (action === 'getCourseById') {
        ...
        const c = rows[0];
        return res.status(200).json({ ...c, price: parseFloat(c.price), modules: c.modules_json || [] });
    }
