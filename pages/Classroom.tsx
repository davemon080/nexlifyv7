...
export const Classroom: React.FC = () => {
  ...
  const handlePostQuestion = async () => {
    ...
    try {
        ...
        setStudentQuestion('');
        showToast("Question sent to tutor", 'success');
        loadLessonQuestions();
    } catch(e) {
    ...
    <div className="flex h-[calc(100vh-80px)] overflow-hidden relative bg-[#0E0E0E] flex-col md:flex-row">
      ...
      <div className="flex-1 flex flex-col w-full relative h-full">
        <div className="px-4 md:px-8 py-4 flex items-center justify-between border-b border-[#444746] bg-[#131314]/95 backdrop-blur-xl sticky top-0 z-20">
            ...
            <div className="flex gap-2 bg-[#1E1F20] p-1 rounded-full border border-[#444746] hidden sm:flex">
                <button onClick={() => setActiveTab('content')} className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase transition-all ${activeTab === 'content' ? 'bg-[#A8C7FA] text-[#062E6F]' : 'text-[#8E918F] hover:text-[#E3E3E3]'}`}>Lesson</button>
                <button onClick={() => setActiveTab('support')} className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase transition-all ${activeTab === 'support' ? 'bg-[#A8C7FA] text-[#062E6F]' : 'text-[#8E918F] hover:text-[#E3E3E3]'}`}>Support</button>
            </div>
        ...
        <div id="classroom-content" className="flex-1 overflow-y-auto p-4 md:p-10 no-scrollbar pb-32">
            <div className="max-w-4xl mx-auto space-y-6 md:space-y-10">
                {activeTab === 'content' ? (
                    ...
                    <div className="space-y-8 animate-in slide-in-from-right duration-300">
                        <div className="text-center mb-8">
                            <h2 className="text-3xl font-bold text-[#E3E3E3]">Tutor Support</h2>
                            <p className="text-[#8E918F]">Have a question about this lesson? Ask your instructor directly.</p>
                        </div>

                        <Card className="p-8 bg-[#131314] border-[#444746]">
                            <Textarea 
                            ...
                            <div className="flex justify-end">
                                <Button icon={Send} onClick={handlePostQuestion} isLoading={isPosting} disabled={!studentQuestion.trim()}>
                                    Submit Question
                                </Button>
                            </div>
                        </Card>
                        ...
                        <div className="space-y-4">
                            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-[#A8C7FA]">Previous Inquiries</h3>
                            {lessonQuestions.length === 0 ? (
                                <div className="p-12 text-center text-[#8E918F] bg-[#1E1F20]/30 rounded-3xl border border-dashed border-[#444746]">
                                    No questions posted for this lesson yet.
                                </div>
                            ) : (
                                lessonQuestions.map((q) => (
                                    <Card key={q.id} className="p-6 border-[#444746]">
                                        <div className="flex items-start gap-4 mb-4">
                                            <div className="w-8 h-8 rounded-full bg-[#1E1F20] flex items-center justify-center shrink-0 border border-[#444746]">
                                                <User className="w-4 h-4 text-[#8E918F]" />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-center mb-1">
                                                    <span className="font-bold text-sm text-[#E3E3E3]">{q.studentName}</span>
                                                    <span className="text-[10px] text-[#5E5E5E] font-bold">{new Date(q.createdAt).toLocaleDateString()}</span>
                                                </div>
                                                <p className="text-sm text-[#C4C7C5] leading-relaxed italic">"{q.question}"</p>
                                            </div>
                                        </div>

                                        {q.reply && (
                                            <div className="mt-4 pl-6 border-l-2 border-[#A8C7FA]/30 py-2">
                                                <div className="flex items-center gap-2 mb-2">
                                                    <Badge color="blue" className="text-[9px] px-1.5 py-0">Tutor Response</Badge>
                                                    <span className="text-[10px] text-[#5E5E5E] font-bold">{new Date(q.repliedAt!).toLocaleDateString()}</span>
                                                </div>
                                                <p className="text-sm text-[#A8C7FA] leading-relaxed font-medium">{q.reply}</p>
                                            </div>
                                        )}
                                        {!q.reply && (
                                            <div className="mt-4 pl-6 py-2">
                                                <div className="flex items-center gap-2 text-[#8E918F]">
                                                    <Loader2 className="w-3 h-3 animate-spin" />
                                                    <span className="text-[10px] font-bold uppercase tracking-widest">Awaiting Instructor Response...</span>
                                                </div>
                                            </div>
                                        )}
                                    </Card>
                ...
                {activeLesson.fileUrl && activeTab === 'content' && (
