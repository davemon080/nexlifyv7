  ...
  const handleEnroll = async () => {
    ...
    if (!user) {
        showDialog({
          ...
          message: 'Please log in to your account to enroll in Nexlify Academy courses.',
    ...
    const completeEnrollment = async (reference?: string) => {
        try {
            ...
            if (reference) await recordTransaction(user.id, 'course_enrollment', course.id, course.price, reference);
            setIsEnrolled(true);
            showToast("Enrollment successful!", 'success');
            navigate(`/classroom/${course.id}`);
        } catch (e) {
            showToast("Enrollment failed. Contact support.", 'error');
        } finally {
            setEnrolling(false);
        }
    };
    ...
    if (course.price > 0) {
        ...
        if (!PaystackPop) {
            showDialog({ title: 'Payment Error', message: 'Payment gateway is currently blocked or loading. Please refresh the page.', onConfirm: () => {} });
            setEnrolling(false);
            return;
        }
        ...
        try {
            const handler = PaystackPop.setup({
                key: paystackKey, email: user.email, amount: Math.ceil(course.price * 100), currency: 'NGN',
                ref: `nex-course-${Date.now()}`,
                callback: function(response: any) { completeEnrollment(response.reference); },
                onClose: function() { showToast("Transaction cancelled", 'info'); setEnrolling(false); },
            });
            handler.openIframe();
        } catch (error: any) {
  ...
  return (
    <div className="min-h-screen pb-20">
      <div className="bg-[#1E1F20] py-16 border-b border-[#444746]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row gap-12 items-center">
          <div className="flex-1">
            ...
            <p className="text-xl text-[#C4C7C5] mb-8 leading-relaxed">{course.description}</p>
            <div className="flex gap-4">
              {isEnrolled ? (
                <Button size="lg" className="bg-[#6DD58C] text-[#0F5223] hover:bg-[#85E0A3]" onClick={() => navigate(`/classroom/${course.id}`)}>Go to Classroom</Button>
              ) : (
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <h2 className="text-3xl font-bold text-[#E3E3E3] mb-10 text-center">Program Curriculum</h2>
        <div className="space-y-6">
          {course.modules.map((module, index) => (
            <Card key={module.id} className="p-6 flex items-start gap-6 hover:bg-[#2D2E30] transition-colors">
              <div className="w-12 h-12 rounded-full bg-[#131314] flex items-center justify-center flex-shrink-0 border border-[#444746] text-[#A8C7FA] font-bold">{index + 1}</div>
              <div className="flex-1">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-lg font-bold text-[#E3E3E3]">{module.title}</h3>
                  <span className="text-xs text-[#8E918F] bg-[#131314] px-3 py-1 rounded-full border border-[#444746]">{module.lessons.length} Lessons</span>
                </div>
